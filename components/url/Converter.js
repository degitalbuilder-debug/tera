...
import LoadingVideo from "@/components/download/LoadingVideo"
import NativeBanner from "@/components/ads/adsterra/NativeBanner"
export default function Converter({ token, url }) {
      ...
      {error && (
        <div className="flex flex-col items-center justify-center text-center gap-4 p-6 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 shadow-md mt-6">
          <p className="text-red-600 dark:text-red-400 font-semibold text-lg">
            There is No File or Please Try again.
          </p>
          <button
      ...
      {!loading && !error && selectedVideo && (
        <>
          ...
          <div className="w-full mt-10">
            ...
            <p className=" text-gray-800  mb-6 text-center">Total <strong>{videoList.length}</strong> File Found.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {videoList.map((video, index) => (
                <Link key={video.fs_id || index} href="#video" className="block">
                  ...
                  >
                    {/* Thumbnail */}
                    <div className="w-full relative aspect-video overflow-hidden bg-gray-200 dark:bg-gray-900">
                      <img
                        src={video.thumbnail}
                        alt={video.name}
  ...
  <Play className="w-6 h-6 text-white" />
                    ...
                    <div className="p-4 flex flex-col items-start text-left w-full">
                      ...
                      <p
                        className={`text-sm mt-1 ${selectedVideo.fs_id === video.fs_id
                            ? "text-gray-100"
